...
## 2026-09-04 KOBITO サーバークラウド LP(`/products/KOBITO`)で見つけた点
...
- ページは `views/templates/products/kobito.html`。イベントページと同じ独立ページ方式
  ...
  確定した構成・方針は docs/DECISIONS.md D-53、経緯は docs/discussion/discussion-2026-09-04-kobito-lp.md。
...
- **ヘッダーの `aspect-ratio: 3/2` + `max-height` は横長画面で右端が白くなる。** 高さが上限に
  当たると CSS の制約転移で幅が縮む(幅 = max-height × 比率)。画像サイズの問題ではない。
  NNTM のトップと同じ `height: 80vh` + `background-size: cover` に変更して解消(c013356)。
  背景写真は NNTM に合わせて 2600px(beach.jpg 361KB。NNTM は 2600px/507KB)。
- 初版は行政向け資料の機能一覧表・イベントページの見出し様式(WHAT/WHY)・チャット図を持ち込んで
  差し戻された。全面再設計の2版は「さっきの版の方がまし」で撤回し、初版に戻して
  ヘッダー・図・コピーだけ差し替えた。見た目が崩れたら版ごと戻す(GUIDELINES 2026-08-06)の実例。
- スクリーンショット(claude-in-chrome)はこの機では描画途中・空白で撮れることが多く、
  ウィンドウのリサイズも効かない。幅の検証は `hero.offsetWidth === innerWidth` のように JS の
  実測値で行った。
- **文言のうちオーナー未決のまま LP に載せた項目**(要確認): 2年目以降の保守料金
  (「別途(年額・お見積り)」と表記)、Lite の価格(買取60万と「初期42万+月1.5万×12(年契約)」を併記)、
  「資料をお送りします」(一次資料は未作成)、補助金は「登録申請中」と表記、電話番号は未記載(050 未契約)。

## 2026-09-05 route_sweep ゴールデン: /filedrop がオーナー機で 404(期待 200)

- `pytest tests/test_route_sweep.py` がオーナー機で 1 件失敗。差分は `/filedrop` のみ
...
## 2026-09-11 イベントページ philsemi2609 プロフィール文言の復元・修正(実測)

- 9/1 のコミット 08a50bf で「東京大学大学院…修士課程を…修了。」の一文が「主な論文に…」に置き換わっていた。
  `git log -p -S'大学院' -- <file>` で消えた地点を特定できる。復元後にオーナー原文で差し替え、最終形は
  「代表的な論文に「…計算言語β」(arXiv リンク)(2022)。東京大学大学院学際情報学府修士課程修了。人口の上位２％の…MENSA会員。」。
- ローカル開発サーバー(127.0.0.1:5000)は前のセッションが起動したまま残ることがある(PID は `lsof -nP -iTCP:5000 -sTCP:LISTEN`)。
  同じツリーを配信していればそのまま使える(デバッグモードでテンプレート編集は即反映)。ControlCenter(AirPlay)も *:5000 を
  listen しているので、python の行だけ見る。
- 手順は「ローカルで 1 点ずつ反映確認 → オーナー ok → まとめて 1 コミット → develop → staging」。ok は直前に
...
## 2026-09-18 未対応: KOBITO ページ FAQ の移行費が旧表記のまま

- 料金セクションの表・説明文は同日の変更で「既存サイトを移行する場合別途費用〜目安として50万円~」に
  更新し本番反映済み(コミット b09d7b2)。だが FAQ「いま使っているサイトはそのまま任せられますか?」の
  回答は旧表記「目安50万〜100万円」のまま残っている(kobito.html 639行目付近)。
- あわせて新説明文の「50万円~」は指示原文どおり半角チルダ。ページ内の他箇所は全角「〜」。
- どちらもオーナーの指示待ち(無断で原稿を変えないルールにより記録のみ)。
