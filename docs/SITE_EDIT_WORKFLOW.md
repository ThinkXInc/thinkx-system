...
- 共有セッションは worktree 隔離された編集・staging 確認まで。
  ...
  **注記(D-50・2026-08-07)**: 現行の承認レベル **L2b** でもこのゲートは維持される。
  マージはオーナーのアカウントでしか行えないため、URL 共有相手は本番に到達できない。
  ただし L2b では staging から `develop` / `release/*` を push できるので、
  **共有相手は staging の内容を変えられる**(本番には出せない)。
  L2a 以下へ下げるとゲート自体が消えるので、その場合は共有中に限り L3 へ戻すこと。
