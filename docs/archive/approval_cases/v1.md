# thinkx-system/docs/archive/approval_cases/v1.md

# 承認ケース集(コーパス) v1 【凍結 2026-09-17・archive・非規範】

**v1 は構築完了により凍結した。** 構築物(`hooks/check_git_command.py`・`infra/scripts/stg.py`)は稼働中。
運用中の新規事例は `docs/approval_cases_v2.md` に貯める。「判定方式の割り当て」表の現行版も v2 側が持つ。
以下は凍結時点の内容そのまま(旧パス `docs/approval_cases_v1.md`)。
