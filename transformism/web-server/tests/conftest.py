...
#
# S2-3: transformism をテストプロセスで import 可能にする配線(thinkx S-0b と同型)。
...
#
# cwd を web-server に固定する理由(F-S2-11): (a) jinja loader が 'views/templates' の相対パス、
# (b) libcommon は名前空間パッケージであり cwd=web-server でないと workspace-root の原本
#     libcommon を誤ロードする。
...
# テストをどこから起動しても本番同等にテンプレ解決・vendored libcommon 解決できるよう
# cwd を web-server に固定する。
os.chdir(_WEBSERVER)
