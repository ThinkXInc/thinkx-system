...
#
# S2-3 (b): 全 GET ルートを Flask test client で総なめし、(ルール, ステータス) を凍結する。
# 200/302/404/500 いずれも「現状(vendoring 後)」として記録する(本文は非決定なので
# status のみ)。このゴールデンが S2-4 スモークのオラクル。
# thinkx/web-server/tests/test_route_sweep.py と同一ロジック。
