...
#  deploy_tick.sh は /usr/local/bin へ複製する(git が実行中のスクリプトを書き換えないため)。

echo "== deploy timer: ENVX=$ENVX =="

sudo install -m 0755 /src/thinkx-system/infra/run/deploy_tick.sh /usr/local/bin/deploy_tick.sh
...
[ -x /usr/local/bin/deploy_tick.sh ] && systemctl is-active "deploy-timer@$ENVX.timer" >/dev/null && printf '\033[32mOK: deploy timer (%s) 稼働中\033[0m\n' "$ENVX" || printf '\033[31mFAIL: deploy timer が起動していない\033[0m\n'
