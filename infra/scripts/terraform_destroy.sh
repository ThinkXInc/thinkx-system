#!/usr/bin/env bash
# thinkx-system/infra/scripts/terraform_destroy.sh   【分類: 変更系(環境を破壊する・オーナー承認前提)】
#
...
#           bash infra/scripts/terraform_destroy.sh staging ~/Sources/infra/terraform
...
terraform_destroy() {
  ...
  [ -f "$tfdir/variables.tf" ] || { printf '%b\n' "${R}FAIL: terraform_destroy $tfdir に variables.tf が無い(terraform ディレクトリを指定する)${Z}"; return 1; }
  ...
  if grep -q "supercom-$other" "$plan_out"; then
    printf '%b\n' "${R}FAIL: terraform_destroy 差分に supercom-$other(逆環境)が混入。中止${Z}"; return 1
  fi
  if ! grep -q "to destroy" "$plan_out"; then
    printf '%b\n' "${Y}WARN: terraform_destroy 破壊対象 0 件(state が空か環境違い)。何もしない${Z}"; return 0
  fi
  ...
  terraform -chdir="$tfdir" destroy -input=false -lock-timeout=10s -auto-approve "${varopt[@]}" \
    && printf '%b\n' "${G}OK: terraform_destroy $env($tfdir)を破壊完了。EIP は解放済み${Z}" \
    || { printf '%b\n' "${R}FAIL: terraform_destroy $env destroy が失敗。上の出力を確認${Z}"; return 1; }
}
