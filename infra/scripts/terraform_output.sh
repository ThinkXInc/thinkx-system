#!/usr/bin/env bash
# thinkx-system/infra/scripts/terraform_output.sh   【分類: 観測系(インフラ無変更。workspace の選択のみ行う)】
#
...
terraform_output() {
  ...
  terraform -chdir="$tfdir" workspace select "$ws" > /dev/null 2>&1 || { printf '%b\n' "${R}FAIL: terraform_output workspace $ws が無い(先に terraform_apply.sh $env)${Z}" >&2; return 1; }
