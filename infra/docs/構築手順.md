...
```
bash infra/scripts/apply_env.sh $ENVX
```

## 2. ssh alias を新 IP に(1分)

```
bash infra/scripts/env_output.sh $ENVX
```
...
```
LB_IP=$(bash infra/scripts/env_output.sh $ENVX lb_public_ip)
...
```
bash infra/scripts/env_output.sh staging lb_public_ip
```
