...
作成日: 2026-09-06 改訂: 2026-09-17(オーナー裁定 4 点を反映) / 状態: **下書き**(実行者が起こした案。オーナーが採用するときに `STAGING_POWER_PLAN.md` へ改名して正本にする)
...
```
https://thinkxinc.com/remote_control/                 (本番の Flask・Basic 認証)
KOBITO(ロゴ)
...
- 本番の Flask に `/remote_control/` を置き、**`/src/thinkx-system/infra/claude_connect/index.html` をそのまま返す**
...
- 接続ページが呼ぶ API(`state` / `session` / `code` / `deploy`)は**相対パス**にする。staging で直接開けば `/connect/state` に、
  本番の `/remote_control/` で開けば `/remote_control/state` に飛ぶ。本番の Flask は `/remote_control/<api>` を `https://staging.thinkxinc.com/connect/<api>` へ
  中継する(requests 2.32.5 は thinkx に導入済み)。認証は本番 web の固定 IP で通る(大原則 5)ので、本番は staging の認証情報を持たない。
...
- 本番 web から `https://staging.thinkxinc.com/connect/state` に到達できる(公開経路・staging 稼働時。P-1b までは 401 が返れば到達 OK)。
- 本番 web の EIP が固定(D-53 台帳)であること: `terraform_output.sh prod web_public_ip`。
...
thinkx/web-server/main.py         /remote_control/(index.html を返す) /remote_control/power/{state,start,stop} /remote_control/{state,session,code,deploy}(中継)
                                  Basic 認証。staging では 404
thinkx/web-server/config.py / .env   REMOTE_BASIC_AUTH_USER/PASS(本番のページの認証だけ)
loadbalancer/conf.d/staging.thinkxinc.com.conf   location /connect/ に satisfy any + allow <本番 web EIP>(中継の通り道)
thinkx/web-server/tests/golden/route_sweep.json   /remote_control/ を追加(本番 401・staging 404)
infra/docs/SECURITY.md / DECISIONS.md   例外の記録(人間)
infra/runbooks/claude-connect.md        入口が本番 /remote_control/ になったことと中継の説明
```
...
### P-1b staging LB: /connect/ を本番 web の IP からは認証なしで通す
- 変更: `loadbalancer/conf.d/staging.thinkxinc.com.conf` の `location /connect/` に `satisfy any; allow <本番 web EIP>; deny all;` を足す
  (`auth_basic` は server ブロックのものがそのまま掛かる。人は Basic 認証、本番 web は IP で通る)。
- 完了条件: Mac から `/connect/state` は 401 のまま。本番 web から `curl https://staging.thinkxinc.com/connect/state` が 200 で state を返す。
- 戻し方: 3 行を消して reload。
- コミット: `feat(infra): staging LB — /connect/ を本番 web の IP からは認証なしで通す(中継用)`

### P-2 IAM ロールと IMDSv2(terraform・本番のみ)
- 変更: iam.tf に `aws_iam_role.web`(EC2 が assume)、`aws_iam_role_policy.web_staging_power`(3 操作・タグ条件)、
  ...
  .tf は両 env 共有(envs/*/instances.tf は同じファイルへの symlink)なので、`var.env == "prod"` のときだけ profile を付ける条件を書き、
  **staging の web にはロールを付けない**(オーナー裁定 2026-09-17)。
...
- 変更: main.py に `/remote_control/`(index.html を返す)、`/remote_control/power/state`(2 台の state と起動時刻)、
  `/remote_control/power/start` / `/remote_control/power/stop`(POST・引数なし)、`/remote_control/state|session|code|deploy`(staging へ中継。deploy は
...
- 完了条件: 本番で `curl -u … https://thinkxinc.com/remote_control/` が 200、認証なしで 401、staging で 404。
  `/remote_control/state` が staging の state をそのまま返す。ルートゴールデン green(追加行込み)。
- 戻し方: ルートとテンプレート参照の削除、ゴールデンの行削除。
- コミット: `feat(thinkx): /remote_control/ — KOBITO リモコンの本番入口(staging 電源 + 接続ページの中継・Basic 認証・本番のみ)`

### P-4 最終検証(スマホのみ)
- 停止中の staging に対して `https://thinkxinc.com/remote_control/` から: (a) 電源カードだけが出る → (b) 起動 → 準備中の段階表示 →
...
- SECURITY.md に例外(web のロール・範囲・理由)、DECISIONS に D-xx、運用.md の STOP/START 節と Claude Code Session 節に URL、
  runbook に「入口は本番 `/remote_control/`、staging の `/connect/` は予備」。SECURITY/DECISIONS への追記は人間。実行者は文面を提案する。
...
## 裁定済み(2026-09-17・オーナー)

- ロールは本番の web だけ(staging の web には付けない)。
- staging LB は本番 web の固定 IP を Basic 認証なしで通す(`satisfy any`)。本番の .env に staging の認証を置かない。
- 入口は `/remote_control/`。
- SECURITY.md「web: ロールなし(現状維持)」は 2026-07-12(旧 infra 2064db2)の現状記述で、禁止ではない(次行に将来オプションとして
  web のロールが予告されている)。当該行を「web: staging 電源用ロールのみ(2026-09)」に書き換える(人間)。
