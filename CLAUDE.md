...
| staging(`web1-stg`) | 本番へ出す | 承認レベル **L2b**(D-50。正本 `docs/DEPLOY_APPROVAL_LEVELS.md`)。`bash infra/scripts/request_production_release.sh`(release 作成・PR・**マージ用 URL の提示**までで止まる) → **オーナーがマージ(これが承認)** → 本番の deploy timer が 60 秒以内に反映 → 本番 URL を実測。実行者はマージしない。オーナー機の `deploy_production_from_staging.sh`(完走型)とは独立した別スクリプト(D-51)。**deploy key に write が付くまでは実行不可**(付いていなければ L3 = オーナー機のみ) |
